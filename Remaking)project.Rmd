...
# tofino l_opt
l_opt_df <-  c("Year","L_start", "L_final", "S", "Likelihood")
...
format_data_for_l_final_vis <- function(l_opt_vec){
  as.data.frame(l_opt_vec) |> na.omit() -> l_opt_df

colnames(l_opt_df) <- l_opt_df[1,]
l_opt_df <- l_opt_df[-1,]

l_opt_df$Likelihood <- as.numeric(l_opt_df$Likelihood,3)
l_opt_df$L_start <- as.numeric(l_opt_df$L_start)
l_opt_df$L_final <- as.numeric(l_opt_df$L_final)
l_opt_df <-  subset(l_opt_df, L_final <= 2 & Likelihood <= 10 & L_final >= 0)


l_opt_df %>%
  group_by(Year) %>%
  slice(which.max(Likelihood)) %>%
  summarize(optimal_L = first(L_final),
            max_likelihood = max(Likelihood)) -> l_opt_vis

l_opt_vis

# plot(optimal_L~Year,tofino_l_opt_vis)
}

tofino_l_opt_vis <- format_data_for_l_final_vis(tofino_l_opt)
```
...
van_l_opt_vis <- format_data_for_l_final_vis(van_l_opt)
```
...
ph_l_opt_vis <- format_data_for_l_final_vis(ph_l_opt)
```
...
#   dplyr::select(Date.Time, Station, Month, Total.Rain..mm.) %>% 
#   mutate(Date.Time = paste0(Date.Time, "-1")) %>% 
#   mutate(Date.Time = myd(Date.Time) ) %>% 
#   filter(Station %in% years1) %>% 
#   dplyr::select(Total.Rain..mm., Station, Month) %>% 
#   spread(Station, Total.Rain..mm.) %>% dplyr::select(-Month) 
# 
...
vic_l_opt_vis <- format_data_for_l_final_vis(vic_l_opt)
```
# Visualization Code
```{r}
vis_df <- rbind(vic_l_opt_vis,ph_l_opt_vis,van_l_opt_vis,tofino_l_opt_vis)
vis_df <-
  cbind(vis_df, data.frame("Station" = c(
    rep("Victoria", 10),
    rep("Port Hardy", 10),
    rep("Vancouver", 10),
    rep("Tofino", 10)
...
vis_df$Year <-  factor(vis_df$Year)
# Assigning colors and symbols based on the 'group' variable
colors <- c("red", "green", "blue", "yellow")
symbols <- c(1, 2, 3, 4)

# Create an empty plot
plot(1, type = "n", xlim = c(1, length(unique(vis_df$Year))), ylim = c(0,max(vis_df$optimal_L)+0.5),
     xlab = "X-axis label", ylab = "Y-axis label", main = "Categorical Plot with Lines and Symbols")

# Loop through unique values of 'group'
i <-  0
for (grp in unique(vis_df$Station)) {
  i <- i + 1
  subset_data <- vis_df[vis_df$Station == grp, ]
  
  # Plot points
  points(subset_data$Year, subset_data$optimal_L, pch = symbols[i], col = colors[i])
  
  # Connect points with lines
  lines(subset_data$Year, subset_data$optimal_L, type = "b", col = colors[i])
}

# Add legend
legend("topright", legend = levels(as.factor(vis_df$Station)), pch = symbols, col = colors)
